...
def xrayFluo(
    ...
    # um-> m: 1e-6
    fraction_absorbed = 1.0 - np.exp(-length * 1e-6 / attenuation_length)
    if verbose:
...
def pulseDuration(t0, L, GVD):
    return t0 * np.sqrt(1.0 + (L / (t0**2 / 2 / np.abs(GVD))))
...
def interpolator(
    x, y, kind="linear", axis=-1, copy=False, bounds_error=False, fill_value=np.nan
):
    from scipy import interpolate

    if kind != "linear":
        print(
            "Warning interp1d can be VERY SLOW when using something that is not liear"
        )
...
def convolveQuad(x, y, xres, yres, useAutoCrop=True, fill_value=0.0, axis=-1):
    print("NOT FINISHED")
    return
...
def convolve(
    x, y, xres, yres, useAutoCrop=True, approximantOrder=None, fill_value=0.0, axis=-1
):
...
def convolveGaussian(x, y, sig=1.0, nPointGaussian=51, fill_value=0.0, axis=-1):
...
def convolveFunc(x, y, func_res, fill_value=0.0, axis=-1):
    from scipy import integrate
...
def convolveFuncParams(x, y, func_res, func_res_pars, fill_value=0.0, axis=-1):
    def tempFunc(xx):
...
def convolve_test(nG=51):
    ...
    t0 = time.time()
    conv_num = convolve(x, y, xG, G, fill_value=0.0)
    print("Num:", time.time() - t0)
    t0 = time.time()
    conv_poly = convolve(x, y, xG, G, approximantOrder=10, fill_value=0.0)
    print("Num:", time.time() - t0)
    t0 = time.time()
    conv_fun = convolveFuncParams(x, y, gaussian, (0, sig), fill_value=0.0)
...
def conv_gauss_and_exp(x, sig, tau):
    ...
    return (
        0.5
        * np.exp(-(2 * tau * x - sig**2) / 2 / tau**2)
        * (1 - erf((-tau * x + sig**2) / sqrt2 / tau / sig))
    )
...
def writev(fname, x, Ys, form="%+.6g", sep=" ", header=None, headerv=None):
    ...
    if Ys.shape[0] == nx:
        ...
        raise MCError(
            "dimension of x (%d) does not match any of the dimensions of Ys (%d,%d)"
...
def writeMatrix(fout, M, x, y, form="%+.6g", sep=" ", header=None):
    ...
    if (nx == len(x)) and (ny == len(y)):
        ...
        e += (
            "shapes of M, x, y: " + str(M.shape) + " " + str(len(x)) + " " + str(len(y))
        )
        raise MCError(e)
...
def rebinOLD(bins_edges, x, *Y):
    ...
    for i in range(n):
        ...
        outX[i] = (bins_edges[i] + bins_edges[i + 1]) / 2.0
...
def rebinTODO(a, shape):
    ...
    if len(shape) != ndim:
        print(
            "Error, asked to rebin a %d dimensional array but provided shape with %d lengths"
            % (ndim, len(shape))
        )
        return None
    nout = []
...
def MedianAndSigma(a):
    ...
    sigma = 1.4826 * MAD
    # this assumes gauss distribution
    return (median, sigma)
...
def iminuitClass(modelFunc):
    ...
    class iminuitFit(object):
        def __init__(self, x, data, init_pars, err=1.0):
            self.x = x
            self.data = data
            self.err = err
        ...
        for t1 in colors.keys():
            for seq in colors[t1].keys():
                for nc in colors[t1][seq].keys():
                    colors[t1][seq][nc] = (
                        np.asarray(colors[t1][seq][nc]["Colors"]) / 255.0
                    )
...
def colors_example_nature(nature, title="colors"):
    ...
    fig, ax = plt.subplots(nSchemes, nSequenceMax, sharex=True, sharey=True, num=title)
